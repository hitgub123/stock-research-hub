import datetime
from monitor import in_push_window, is_us_dst, us_open_jst, run_monitor, close_summary
...
class FakeClient:
    def __init__(self, docs): self._col = FakeCol(docs)
...
class FakeCol:
    def __init__(self, docs): self.docs=dict(docs)
    def stream(self):
        class _D:
...
def test_run_triggers_and_cooldown():
    store = make_store({"AAPL": {"target": 200.0, "source": "script"}})
    sent = []
    r = run_monitor(store, "hook", dt(2026, 7, 15, 22, 30),
                    fetch=lambda t: 180.0, send=lambda *a, **k: sent.append(a[2]) or True)
    assert r["triggered"] == 1 and r["pushed"] == 1
    assert sent and "AAPL" in sent[0]
    assert "last_notified" in store.get("AAPL")
    # 冷却: 3 天后再跑 → 不推
    r2 = run_monitor(store, "hook", dt(2026, 7, 18, 22, 30),
                     fetch=lambda t: 170.0, send=lambda *a, **k: sent.append(a[2]) or True)
    assert r2["triggered"] == 0 and len(sent) == 1
    # 8 天后 → 再推
    r3 = run_monitor(store, "hook", dt(2026, 7, 23, 22, 30),
                     fetch=lambda t: 150.0, send=lambda *a, **k: sent.append(a[2]) or True)
...
def test_close_summary():
    store = make_store({"AAPL": {"target": 200.0}, "MSFT": {"target": 400.0}})
    sent = []
